<template>
  <fieldset :id="field.id + '-fs'" :class="{ 'required-field': isRequired }" v-show="isVisible">
    ...
    <div v-if="fieldState && fieldState.$dirty && fieldState.$touched" class="form-control-feedback mg-evaluation-error">
      <div class="text-warning small">{{evaluationMessage}}<span v-if="evaluationError.message">: {{ evaluationError.message }}</span></div>
    </div>
  </fieldset>
</template>
